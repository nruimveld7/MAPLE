<script lang="ts">
	...
	const shellTheme = resolveShellTheme({ mode: 'dark' });
	...
	let lastName = 'User';
	let email = 'account.user@example.com';
	...
	function startThemeDrag(event: PointerEvent) {
		...
		themeMode = themeOptions[getSegmentIndexFromPointer(event, themeSlider, themeOptions.length)];

		const move = (moveEvent: PointerEvent) => {
			if (themeSlider.hasPointerCapture(moveEvent.pointerId)) {
				setThemeThumbFromPointer(moveEvent);
				themeMode = themeOptions[getSegmentIndexFromPointer(moveEvent, themeSlider, themeOptions.length)];
			}
		};
		...
		const done = (doneEvent: PointerEvent) => {
			...
			themeMode = themeOptions[getSegmentIndexFromPointer(doneEvent, themeSlider, themeOptions.length)];
			themeThumbX = null;
	...
	function getSegmentIndexFromPointer(event: PointerEvent, slider: HTMLElement, optionCount: number) {
		...
		const x = clamp(event.clientX - rect.left, 0, rect.width);
		const segmentWidth = rect.width / optionCount;
		const middleIndex = Math.floor(optionCount / 2);

		const distances = Array.from({ length: optionCount }, (_, index) => {
			const center = segmentWidth * index + segmentWidth / 2;
			return Math.abs(x - center);
		});

		const minimumDistance = Math.min(...distances);
		const tiedIndexes = distances
			.map((distance, index) => ({ distance, index }))
			.filter(({ distance }) => Math.abs(distance - minimumDistance) < 0.0001)
			.map(({ index }) => index);

		if (tiedIndexes.includes(middleIndex)) return middleIndex;
		return tiedIndexes[0] ?? 0;
	}
	...
	function startFormatDrag(kind: ColorKind, event: PointerEvent) {
		...
		const move = (moveEvent: PointerEvent) => {
			if (slider.hasPointerCapture(moveEvent.pointerId)) {
...
<svelte:head>
	<title>Account | Auth Portal</title>
</svelte:head>
							...
							{#if openColorPicker === 'primary'}
								<div class="color-popover" role="dialog" aria-label="Primary color picker" on:click|stopPropagation>
									<div class="color-popover-header">
										...
										<button class="btn" type="button" on:click={() => (openColorPicker = null)}>Close</button>
									</div>
